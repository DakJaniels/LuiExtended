...
-- Blink animation OnStop handler
function LUIE_SpellCastBuffIcon_BlinkAnimation_OnStop(self)
    -- Get the animated control from the first animation
    local firstAnimation = self:GetFirstAnimation()
    if firstAnimation then
        local control = firstAnimation:GetAnimatedControl()
        if control then
            control:SetAlpha(1)
        end
    end
