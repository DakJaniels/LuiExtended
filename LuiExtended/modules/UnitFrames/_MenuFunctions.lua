    ...
    if IsConsoleUI() and LUIE.ConsoleMoverHelper then
        ...
        if EditModeController and state then
            if not EditModeController:IsEditModeActive() then
                EditModeController:SetEditModeActive(true, "UnitFrames")
            end
        end
        ...
        for _, unitTag in pairs(
            ...
            if UnitFrames.CustomFrames[unitTag] and UnitFrames.CustomFrames[unitTag].tlw then
                ...
                -- Set up gamepad handler
                MoverHelper.SetupGamepadHandler(tlw, "unitFrames", function (control, left, top)
                    UnitFrames.SV[control.customPositionAttr] = { left, top }
                end)

                -- Update control state
                MoverHelper.UpdateControlState(tlw, identifier, state)
            end
        end

        -- Unlock buffs for Player (preview control is created in SpellCastBuffs module)
        -- When locked to unit frames (default), buffs/debuffs move with the unit frame
        -- When not locked, they're handled by SpellCastBuffs.SetMovingState
        if UnitFrames.CustomFrames["player"] and UnitFrames.CustomFrames["player"].tlw then
            ...
            if lockToUnitFrames then
                -- Buffs/debuffs are locked to unit frame - they move with it, just show/hide previews
                if UnitFrames.CustomFrames["player"].buffs and UnitFrames.CustomFrames["player"].buffs.preview then
                    UnitFrames.CustomFrames["player"].buffs.preview:SetHidden(not state)
                end
                if UnitFrames.CustomFrames["player"].debuffs and UnitFrames.CustomFrames["player"].debuffs.preview then
                    UnitFrames.CustomFrames["player"].debuffs.preview:SetHidden(not state)
                end
            else
                -- Not locked to unit frames - handled by SpellCastBuffs.SetMovingState
                -- Just ensure previews are visible if SpellCastBuffs has them enabled
                if UnitFrames.CustomFrames["player"].buffs and UnitFrames.CustomFrames["player"].buffs.preview then
                    UnitFrames.CustomFrames["player"].buffs.preview:SetHidden(not state)
                end
                if UnitFrames.CustomFrames["player"].debuffs and UnitFrames.CustomFrames["player"].debuffs.preview then
        ...
        -- Unlock buffs and debuffs for Target (preview controls are created in SpellCastBuffs module)
        if UnitFrames.CustomFrames["reticleover"] and UnitFrames.CustomFrames["reticleover"].tlw then
            local lockToUnitFrames = LUIE.SpellCastBuffs and LUIE.SpellCastBuffs.SV and (LUIE.SpellCastBuffs.SV.lockPositionToUnitFrames == nil or LUIE.SpellCastBuffs.SV.lockPositionToUnitFrames)

            if lockToUnitFrames then
                -- Buffs/debuffs are locked to unit frame - they move with it, just show/hide previews
                if UnitFrames.CustomFrames["reticleover"].buffs and UnitFrames.CustomFrames["reticleover"].buffs.preview then
                    UnitFrames.CustomFrames["reticleover"].buffs.preview:SetHidden(not state)
                end
                if UnitFrames.CustomFrames["reticleover"].debuffs and UnitFrames.CustomFrames["reticleover"].debuffs.preview then
