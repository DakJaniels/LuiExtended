...
local function AddPotionCooldownToFrame(frameData, isRaid)
    ...
    if not Settings.GroupPotionCooldowns.enabled then return end
    ...
    if not frameData.potionCooldown then
        ...
        local iconSize
        if Settings.GroupCombatStats.enabled and Settings.GroupCombatStats.showUltimate then
            iconSize = isRaid and Settings.GroupCombatStats.ultIconRaidSize or Settings.GroupCombatStats.ultIconGroupSize
        else
            iconSize = isRaid and Settings.GroupPotionCooldowns.potionIconRaidSize or Settings.GroupPotionCooldowns.potionIconGroupSize
        end

        local offsetX = isRaid and Settings.GroupPotionCooldowns.potionIconRaidOffsetX or Settings.GroupPotionCooldowns.potionIconGroupOffsetX
        local offsetY = isRaid and Settings.GroupPotionCooldowns.potionIconRaidOffsetY or Settings.GroupPotionCooldowns.potionIconGroupOffsetY
        ...
        -- Anchor after ultimate icons if they exist, otherwise anchor to health bar
        ...
        if Settings.GroupPotionCooldowns.showRemainingTime then
...
local function UpdatePotionCooldownDisplay(unitTag, potionData)
    ...
    if potionData.isOnCooldown then
        ...
        if Settings.GroupPotionCooldowns.showRemainingTime and label then
    ...
    if Settings.GroupPotionCooldowns.showRemainingTime then
        EVENT_MANAGER:RegisterForUpdate("LUIE_GroupPotionCooldowns_Update", 1000, function ()
            ...
            for unitTag, potionData in pairs(potionDataCache) do
                if potionData and UnitFrames.CustomFrames[unitTag] and UnitFrames.CustomFrames[unitTag].potionCooldown then
                    UpdatePotionCooldownDisplay(unitTag, potionData)
                end
            end
        end)
...
function GroupPotionCooldownsManager.SetupFrames()
    ...
    for i = 1, 4 do
        ...
        if frameData and not frameData.potionCooldown then
            AddPotionCooldownToFrame(frameData, false)
        end
    end
    ...
    for i = 1, 12 do
        ...
        local frameData = UnitFrames.CustomFrames[unitTag]
        if frameData and not frameData.potionCooldown then
            AddPotionCooldownToFrame(frameData, true)
        end
    end
end
