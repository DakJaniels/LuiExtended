...
--- Does not move the alert TLW; the game HUD Editor owns frame position.
...
--- Apply LUIE.SV.AlertFrameAlignment to keyboard/gamepad alert notification text.
...
--   HUD_MANAGER:RegisterKeyboardElement(control, displayName, config, options)
--   HUD_MANAGER:RegisterGamepadElement(control, displayName, config, options)
-- Element requirements (ZO_HUDManager_Element:Initialize):
--   primary anchor present, no secondary anchor, control.hudElementRef for editor box
-- PreLoadSettings is the last chance before SV load (hudtracker_manager.lua).
-- -----------------------------------------------------------------------------
...
--- Collapse dual/stretch anchors to a single GuiRoot TOPLEFT so HUD_MANAGER asserts pass.
--- @param control Control
--- @return boolean ready
--- @return ZO_Anchor|nil defaultAnchor
...
--- @class LUIE_UnmanagedHudElementSpec
--- @field control Control|nil
--- @field displayName string|function
--- @field registerKeyboard boolean|nil
--- @field registerGamepad boolean|nil
--- @field isValid (fun(element: any): boolean)|nil

--- Controls previously in LUIE Unlock movers that are not registered by ZOS HUD code.
--- @return LUIE_UnmanagedHudElementSpec[]
local function GetUnmanagedHudElementSpecs()
    return
    {
        ...
        {
            -- Meter child of ZO_ObjectiveCapture (objectivecapturemeter.xml)
        ...
        {
            -- CreateControlFromVirtual(ZO_Tutorial, ..., "HudInfoTipKeyboard") -> ZO_TutorialHudInfoTipKeyboard
...
--- @param control Control
--- @param displayName string|function
--- @param defaultAnchor ZO_Anchor
--- @param isValid (fun(element: any): boolean)|nil
--- @param registerKeyboard boolean
--- @param registerGamepad boolean
...
--- Register default UI frames that ZOS does not pass to HUD_MANAGER.
--- Safe to call more than once; no-ops after the first successful pass.
...
function Unlock.InstallUnmanagedHudElementRegistration()
    ...
    -- hudmanager.lua fires PreLoadSettings on ADD_ONS_LOADED before reading ZO_HUDManager SV.
